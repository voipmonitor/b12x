...
def s0_load_q_bf16_to_smem(
    ...
    *,
    ...
    ``q_rope_stride`` / ``barrier_threads`` mirror ``s0_quantize_q_to_smem``
    (the NVFP4 path never runs the native-H8/H16 modes, so ``barrier_threads``
    stays 0 in practice; it is kept for signature parity with the caller).
    """
    bar_kw = dict(
        barrier_id=barrier_id,
        number_of_threads=(barrier_threads if barrier_threads else num_threads),
...
def s6_xv_nope(
    ...
    *,
    ...
    sm_p_stride: cutlass.Constexpr = 0,  # NVFP4 P row stride (elems); 0 -> BI
):
...
def s6_xv_nope_nvfp4_bf16(
    ...
    *,
    ...
    sm_p_stride: cutlass.Constexpr = 0,  # P row stride (bf16 elems); 0 -> BI
):
    ...
    ``sm_p_stride``: decode's dedicated sm_p_full region is unpadded (stride BI,
    the default); the MG prefill tile aliases the dead W_FP8 region with the
    padded ``BI + 8`` stride (smem_mg ``sm_p_full_stride``).
    """
